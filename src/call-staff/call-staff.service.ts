...
import { CreateCallStaffPayBillDto } from './dto/create-call-staff-pay-bill.dto';
...
export class CallStaffService {
  ...
  async createCallStaffByGroupPayBill(
    createCallStaffPayBillDto: CreateCallStaffPayBillDto,
    group_id: string,
  ): Promise<CallStaffDocument> {
    const newCallStaff = Object.assign(createCallStaffPayBillDto);
    const existingTable = await this.tableService.findTableByName(
      createCallStaffPayBillDto.table,
      group_id,
    );
    if (!existingTable) {
      throw new Error('The table does not exist');
    ...
    const dataSocket = {
      ...
      customer_name: callStaffCreated.customer_name,
      group_id: callStaffCreated.group_id,
      totalBill: callStaffCreated.totalBill,
    };
    await this.eventsGateway.createCallStaffForPay(dataSocket);
    return callStaffCreated;
  }
  ...
  async findAllCallStaffByGroup(groupId: string, time?: number): Promise<any> {
    ...
    const callStaffs = callStaffsNoCashier.filter(
      (callStaff) =>
        callStaff.group_id === groupId && callStaff.totalBill === null,
    );
  ...
  async findAllCallStaffPayBillByGroup(
    groupId: string,
    time?: number,
  ): Promise<any> {
    const callStaffsNoCashier =
      await this.callStaffRepository.findObjectWithoutLimit();
    const callStaffs = callStaffsNoCashier.filter(
      (callStaff) => callStaff.group_id === groupId && callStaff.totalBill,
    );
    if (callStaffs === null || callStaffs.length === 0) {
      return 'No call staff created';
    }
